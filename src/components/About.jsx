            ...
            <div className="rounded-2xl border border-border bg-surface-raised p-6 shadow-sm">
              ...
              <ul className="space-y-3">
                {focus.map((item) => (
                  <li
                    key={item}
                    className="flex gap-3 text-ink-muted before:mt-2.5 before:block before:h-1.5 before:w-1.5 before:shrink-0 before:rounded-full before:bg-accent before:content-['']"
                  >
                    {item}
                  </li>
                ))}
            ...
            <div className="rounded-2xl border border-border bg-surface-raised p-6 shadow-sm">
              ...
              <ul className="space-y-2.5 text-ink-muted">
                {approach.map((item) => (
                  <li
                    key={item}
                    className="border-l-2 border-accent/50 pl-3 leading-snug"
                  >
                    {item}
                  </li>
                ))}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {interests.map((interest) => (
                  <span
                    key={interest}
                    className="rounded-full border border-border bg-surface-raised px-3 py-1.5 text-sm text-ink-muted shadow-sm"
                  >
                    {interest}
                  </span>
                ))}
